-- Pre-registros del organizador (sin fila en auth.users) no deben heredar role organizer.
-- Evita que un jugador invitado entre con permisos de organizador al vincular magic link.
...
BEGIN
  ...
  IF existing_id IS NOT NULL THEN
    ...
    -- Pre-registro (sin auth.users): siempre jugador. Solo cuentas auth previas conservan role.
    IF EXISTS (SELECT 1 FROM auth.users u WHERE u.id = existing_id) THEN
      merged_role := coalesce(nullif(trim(existing_rec.role), ''), user_role);
    ELSE
      merged_role := 'jugador';
    END IF;
