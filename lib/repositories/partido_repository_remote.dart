...
class PartidoRepositoryRemote {
  ...
  /// Suma pendiente de partidos anteriores al indicado (deuda anterior viva).
  Future<double> _sumPendientePartidosAnteriores({
    ...
    final rows = await _client
        ...
        .select('total, monto_pagado, partido_id, partidos!inner(fecha)')
    ...
    for (final row in rows as List) {
      ...
      if (pid == partidoId) continue;

      final partido = SupabaseParse.mapEmbed(map['partidos']);
      final fecha = partido == null
          ? DateTime.fromMillisecondsSinceEpoch(0)
          : SupabaseParse.toDateTime(partido['fecha']);

      final esAnterior = fecha.isBefore(fechaPartido) ||
          (fecha == fechaPartido && pid < partidoId);
  ...
  /// Deuda anterior real para desglose/conciliación (no el snapshot al crear partido).
  Future<double> _saldoAnteriorVivoParaPartido({
    required String jugadorId,
    required int partidoId,
    required DateTime fechaPartido,
    double? saldoHistorico,
  }) async {
    final deudaAnterior = await _sumPendientePartidosAnteriores(
      jugadorId: jugadorId,
      partidoId: partidoId,
      fechaPartido: fechaPartido,
    );
    if (deudaAnterior > 0.005) return deudaAnterior;

    if (saldoHistorico != null && saldoHistorico < -0.005) {
      return roundMoney(saldoHistorico).toDouble();
    }
    return 0;
  }

  Future<DateTime?> _fechaPartido(int partidoId) async {
    ...
    return SupabaseParse.toDateTime(row['fecha']);
  }
  ...
  Future<void> _insertarCostosYDetalles({
    ...
    for (final jugadorId in asistentes) {
      ...
      final rawId = idMap.entries
          ...
          .key;
      final saldoAnterior = saldosAnterioresSnapshot?[rawId] ??
    ...
    unawaited(Future.wait(asistentes.map(reconciliarDetallesJugador)));
  }
  ...
  Future<List<PartidoCompleto>> getCompletosListaResumen(
    ...
    return SupabaseHelpers.guard('Listar partidos (resumen)', () async {
      ...
      for (final id in partidoIds) {
        ...
        result.add(PartidoCompleto(partido: partido, detalles: detalles));
      }
      return result;
    });
  }

  /// Lectura de desglose. [reconciliar] solo para flujos de escritura/validación.
  Future<List<DesgloseJugador>> getDesglose(
    ...
    final fechaPartido = completo.partido.fecha;
    if (reconciliar) {
      await Future.wait(
        asistentes.map((d) => reconciliarDetallesJugador(d.jugadorSupabaseId!)),
      );
    }

    final saldosAnt = await Future.wait(
      asistentes.map((d) async {
        final jid = d.jugadorSupabaseId!;
        return MapEntry(
          jid,
          await _saldoAnteriorVivoParaPartido(
            jugadorId: jid,
            partidoId: partidoId,
            fechaPartido: fechaPartido,
            saldoHistorico: saldosAnteriores[jid],
          ),
        );
      }),
    );
    final saldoPorJugador = Map<String, double>.fromEntries(saldosAnt);

    final desgloses = <DesgloseJugador>[];
    for (final d in asistentes) {
      final jid = d.jugadorSupabaseId!;
      final saldoAnt = saldoPorJugador[jid] ?? 0;
  ...
  Future<List<ResumenJugador>> getResumenJugadores({bool reconciliar = false}) async {
    ...
    if (reconciliar) {
      await Future.wait(ids.map(reconciliarDetallesJugador));
    }

    final statsRows = await _client
        .from('detalles_partido')
        .select('jugador_id, total, monto_pagado, pagado')
        .inFilter('jugador_id', ids)
        .eq('asistio', true);

    final statsPorJugador = <String, ({int partidos, double pendiente})>{};
    for (final id in ids) {
      statsPorJugador[id] = (partidos: 0, pendiente: 0.0);
    }
  ...
  Future<List<DeudaPartidoAnterior>> _fetchPendientesBruto(String jugadorId) async {
    final rows = await _client
        .from('detalles_partido')
        .select('total, monto_pagado, partidos!inner(id, fecha, recinto, sport_type)')
        .eq('jugador_id', jugadorId)
        .eq('asistio', true)
        .eq('pagado', false);

    return _mapPendientesBruto(rows);
  }

  Future<Map<String, List<DeudaPartidoAnterior>>> _fetchPendientesBrutoBatch(
    ...
    final rows = await _client
        .from('detalles_partido')
        .select(
          'jugador_id, total, monto_pagado, partidos!inner(id, fecha, recinto, sport_type)',
        )
    ...
    for (final item in _mapPendientesBrutoConJugador(rows)) {
  ...
  List<DeudaPartidoAnterior> _mapPendientesBruto(List rows) {
    return _mapPendientesBrutoConJugador(rows)
        .map((e) => e.deuda)
        .where((d) => d.montoPendiente > 0)
        .toList()
      ..sort((a, b) => a.fecha.compareTo(b.fecha));
  }

  Iterable<({String jugadorId, DeudaPartidoAnterior deuda})>
      _mapPendientesBrutoConJugador(List rows) sync* {
    for (final row in rows) {
      ...
      final deuda = DeudaPartidoAnterior(
        partidoId: SupabaseParse.toInt(partido['id']),
  ...
  /// Alinea detalles impagos con el saldo_acumulado (como en SQLite local).
    ...
    await SupabaseHelpers.guard('Reconciliar detalles jugador', () async {
      ...
      if (saldo <= 0) {
        final impagos = await _client
            ...
            .eq('asistio', true)
            .eq('pagado', false);
        for (final row in impagos as List) {
  ...
  Future<List<DeudaPartidoAnterior>> getDeudasPartidosAnteriores({
    ...
    return SupabaseHelpers.guard('Deudas partidos anteriores', () async {
      ...
      final bruto = await _fetchPendientesBruto(jugadorId);
      return bruto.where((d) => d.partidoId != partidoActualId).toList();
    });
  }
  ...
  Future<List<DeudaPartidoAnterior>> getPartidosPendientesJugador(
    ...
    return SupabaseHelpers.guard('Partidos pendientes jugador', () async {
      if (reconciliar) await reconciliarDetallesJugador(jugadorId);
      return _fetchPendientesBruto(jugadorId);
    });
  }
  ...
  Future<List<ResumenJugador>> getDeudoresVencidos(int diasMinimos) async {
    ...
    final ids = deudores
        ...
        .toList();
    final pendientesPorJugador = await _fetchPendientesBrutoBatch(ids);
    final ahora = DateTime.now();
  ...
  Future<DesgloseJugador?> getMiDesglosePartido(int partidoId) async {
    return SupabaseHelpers.withTimeout(
      SupabaseHelpers.guard('Mi desglose partido', () async {
        ...
        final fechaPartido =
            await _fechaPartido(partidoId) ?? DateTime.now();
        ...
        try {
          ...
          if (raw is Map) {
            ...
            final saldoAnt = await _saldoAnteriorVivoParaPartido(
              jugadorId: uid,
              partidoId: partidoId,
              fechaPartido: fechaPartido,
              saldoHistorico:
                  SupabaseParse.toDouble(rpcMap['saldo_anterior']),
            );
        ...
        final histRow = await _client
            ...
            .maybeSingle();
        final saldoHist =
            (histRow?['saldo_anterior'] as num?)?.toDouble();
        final saldoAnt = await _saldoAnteriorVivoParaPartido(
          jugadorId: uid,
          partidoId: partidoId,
          fechaPartido: fechaPartido,
          saldoHistorico: saldoHist,
        );
  ...
  Future<List<DetallePartido>> getMisDeudasPendientes({
    ...
    return SupabaseHelpers.guard('Mis deudas', () async {
      ...
      if (reconciliar) {
        await reconciliarDetallesJugador(uid);
      }
      ...
      final lista = await _fetchMisDeudasDirecto(uid);
      return lista
          .where(
            (d) =>
                d.comprobantePendienteValidacion || d.montoPendiente > 0.005,
          )
          .toList();
    });
  }
  ...
  Future<void> eliminarPartido(int id) async {
    await SupabaseHelpers.guard('Eliminar partido', () async {
      ...
      for (final jid in jugadorIds) {
        await _recalcularSaldoJugador(jid);
        await reconciliarDetallesJugador(jid);
      }
    });
  }
  ...
  Future<void> validarComprobantePago({
    ...
    await SupabaseHelpers.guard('Validar comprobante', () async {
      ...
      final saldoAnteriorVivo = await _saldoAnteriorVivoParaPartido(
        jugadorId: jugadorId,
        partidoId: partidoId,
        fechaPartido: fechaPartido,
      );
      final pendienteEnCobro = CobroLogic.pendienteEnCobro(
        saldoAnterior: saldoAnteriorVivo,
        cargoPartido: total,
        montoPagado: montoPagado,
      );
      ...
      final decision = CobroLogic.evaluarValidacionComprobante(
        ...
        pendienteEnCobro: pendienteEnCobro,
      ...
      switch (decision.accion) {
        case ComprobanteValidacionAccion.rechazar:
          ...
          final montoPendiente = pendienteEnCobro > 0.005
              ? pendienteEnCobro
              : roundMoney(total - montoPagado).toDouble().clamp(0.0, double.infinity);
          unawaited(
      ...
      final pendienteCargo =
          roundMoney(total - montoPagado).toDouble().clamp(0.0, double.infinity);
      final aplicarEnDetalle =
          abono >= pendienteCargo ? pendienteCargo : abono;
      final nuevoMontoPagado =
          roundMoney(montoPagado + aplicarEnDetalle).toDouble();
      final cubierto = nuevoMontoPagado >= total - 0.005;
