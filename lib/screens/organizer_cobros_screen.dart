...
class _OrganizerCobrosScreenState extends State<OrganizerCobrosScreen> {
  ...
  Future<void> _load({bool silent = false}) async {
    ...
    try {
      final resumenes =
          await context.repos.getResumenJugadores(reconciliar: false);
      if (mounted) {
        setState(() {
          _resumenes = resumenes;
          _resumen = cobrosResumenDesdeResumenes(resumenes);
          _loading = false;
        });
      }
    } catch (_) {
                    ...
                    child: Row(
                      children: [
                        _FiltroChip(
                          ...
                          onTap: () =>
                              setState(() => _filtro = _FiltroCobros.todos),
                        ),
                        ...
                        _FiltroChip(
                          ...
                          onTap: () =>
                              setState(() => _filtro = _FiltroCobros.conDeuda),
                        ),
                        const SizedBox(width: 8),
                        _FiltroChip(
                          label: l10n.tr('organizerCobrosFilterUpToDate'),
                          selected: _filtro == _FiltroCobros.alDia,
                          onTap: () =>
                              setState(() => _filtro = _FiltroCobros.alDia),
                        ),
                        const SizedBox(width: 8),
                        _FiltroChip(
                          ...
                          onTap: () =>
                              setState(() => _filtro = _FiltroCobros.credito),
                        ),
                      ],
                    ),
                  ),
